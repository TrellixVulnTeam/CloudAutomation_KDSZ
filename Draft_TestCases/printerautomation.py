...
def printer_automation(text):
    ...
    #Check whether the job name as defined is shown in the list. If not found refresh and check again. If present, then Print.
    while True:
        try:
            #text="Hello.txt"
    ...
    while True:
        try:
            ups.regex('In area "resource-id=\'esf.printReleaseUi:id/lvFragMain\'" Do "nothing"')
            break
        except ValueError:
            print("Job not found. Check on cloud portal")
            print_status='True'
            time.sleep(15)
            return print_status
            #Return to Portal to check print job history


#printer_automation("Hello.txt")