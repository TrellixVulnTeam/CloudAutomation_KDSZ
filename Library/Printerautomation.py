...
def printer_automation_latebindcopy(ip,pin,text,state,copy_value):
    ...
    while True:
        try:
            ...
            ups.regex(ups_command)
            ups.regex('Find Widget "resource-id=\'esf.printReleaseUi:id/action_settings\'" do "press"')
            ups.regex('On text "Change Print Settings" Do "press"')
            #ups.regex('Find Widget "resource-id=\'esf.printReleaseUi:id/layout_copybuttons\'" do "press"')

            ups_command_increase='''Find widget "resource-id=\'esf.printReleaseUi:id/btnIncreaseNumCopies\'" Do "verify enabled='{}'"'''.format(state)
            ups_command_decrease ='''Find widget "resource-id=\'esf.printReleaseUi:id/btnDecreaseNumCopies\'" Do "verify enabled='{}'"'''.format(
                state)
            ups_command_text = '''Find widget "resource-id=\'esf.printReleaseUi:id/txtNumCopiesValue\'" Do "verify text='{}'"'''.format(copy_value)

            ups.regex(ups_command_increase)
            ups.regex(ups_command_decrease)
            ups.regex(ups_command_text)
            status_message="Late Bind copy is shown correctly and state is " + state
            print(status_message)
            print_status="True"
            ups.regex('do "press key KEYCODE_BACK"')
            time.sleep(1)
            ...
            status_message="Late Bind is incorrectly set and state is not " + state
            print(status_message)
            print_status="False"
            ups.regex('do "press key KEYCODE_BACK"')
            time.sleep(1)
