...
def create_user():
    ...
    base_url = "https://idp.dev.us.cloud.onelxk.co/rest/users?organization_id="
    ...
    base_url = "https://idp.dev.us.cloud.onelxk.co/rest/role_grants"

    payload = "{ \n\t\"organizationId\": \""+str(org_id[0])+"\"," \
              "\n\t\"principalId\": \""+str(user_id[0])+"\"," \
              "\n\t\"roleName\": \"LPM_PRINT_RELEASE:ROLE_ADMIN\"," \
              "\n\t\"principalType\": \"User\"\n}"
    requests.request("POST", base_url, headers=headers, data=payload)
    return nonadmin
...
def quota_green():
    ...
    # Get Org ID
    org_url = "https://idp.dev.us.cloud.onelxk.co/rest/users/email/cpmautomation@test.onelxk.co"
    payload = {}
    headers = {'Authorization': ''}
    auth_token = str("Bearer " + access_token)
